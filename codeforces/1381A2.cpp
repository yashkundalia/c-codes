...
void solve()
{
    ...
    vector<int> a1;
    vector<int> a2;
    //if(s1[i] == '1') a1.push_back(1);
    //if(s2[i] == '1') a2.push_back(1);
    for(int i=1;i<n;i++)
    {
        if(s1[i] != s1[i-1]) a1.push_back(i);
        if(s2[i] != s2[i-1]) a2.push_back(i);
    }
    if(s1[n-1] == '1') a1.push_back(n);
    if(s2[n-1] == '1') a2.push_back(n);
    a1.insert(a1.end() , a2.rbegin(),a2.rend());
    cout<<a1.size()<<" ";
